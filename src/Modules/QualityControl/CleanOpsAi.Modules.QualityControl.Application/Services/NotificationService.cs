		...
		public class NotificationService : INotificationService
		{
			...
			public async Task HandleSendNotificationAsync(SendNotificationEvent message, CancellationToken ct = default)
			{
				...
				var notification = new AppNotification
				{
					...
					CreatedBy = message.SenderId?.ToString() ?? "system",
					NotificationRecipients = message.Recipients.Select(r => new NotificationRecipient
					{
						Id = _idGenerator.Generate(),
						RecipientType = r.RecipientType,
						RecipientId = r.RecipientId,
				...
				await _notificationRepository.InsertAsync(notification, ct); 
				await _notificationRepository.SaveChangesAsync(ct); 
